...
def calculate_TP(curr_detections, gt_boxes_for_frame):
    TP = 0
    assigned_gt_indices = set()  # To keep track of assigned ground truth indices
    for detection in curr_detections:
        detection_tuple = tuple(detection)
        max_iou = 0
        max_iou_gt_index = None
        for i, gt_box in enumerate(gt_boxes_for_frame):
            iou = calculate_iou(detection, gt_box)
            if iou > max_iou:
                max_iou = iou
                max_iou_gt_index = i
        if max_iou > 0.5 and max_iou_gt_index is not None:
            if max_iou_gt_index not in assigned_gt_indices:
                TP += 1
                assigned_gt_indices.add(max_iou_gt_index)
    return TP
